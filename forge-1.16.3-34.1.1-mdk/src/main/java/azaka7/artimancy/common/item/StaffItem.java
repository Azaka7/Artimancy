...
public class StaffItem extends ToolItem{
	...
	public ITextComponent getDisplayName(ItemStack stack) {
		...
		if(spell != null && !stack.hasDisplayName()) {
			return (new StringTextComponent("")).func_230529_a_(super.getDisplayName(stack)).func_230529_a_(new StringTextComponent(" (")).func_230529_a_(new TranslationTextComponent(spell.getID())).func_230529_a_(new StringTextComponent(")"));
		}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		...
		if(spell != null) {
			...
			tooltip.add(1, (new TranslationTextComponent("artimancy.spell_label")).func_230529_a_(new StringTextComponent(": ")).func_230529_a_(new TranslationTextComponent(spell.getID())));
			tooltip.add(2, (new TranslationTextComponent("artimancy.XPCostLabel")).func_230529_a_(new StringTextComponent(": "+Spells.calcSpellCost(spell, vigor, stack.getItemEnchantability(), autophagy))));
		} else {
	...
	private static Set<Block> getEffectiveOn(){
		...
		for(Block block : ForgeRegistries.BLOCKS.getValues()) {
			if(block.getDefaultState().getMaterial().equals(Material.GLASS)) {
				builder.add(block);
			}
		}
