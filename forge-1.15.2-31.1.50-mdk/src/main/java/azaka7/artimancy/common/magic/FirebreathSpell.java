...
public class FirebreathSpell extends AbstractSpell{
	...
	public boolean castSpell(LivingEntity caster, int power, int focus) {
        ...
        for(Entity entity : entities) {
        	...
        	Vec3d closestOnLOS = eyePos.add(lookDir.scale(dist));//Closest position to the entity on the player's line of sight
		...
		Vec3d unit_a = lookDir.normalize();
		...
		for(int flames = 0; flames < 20 * (power+1); flames++) {
			Vec3d motion = unit_a.scale(1.0+ 0.5*world.rand.nextDouble());
